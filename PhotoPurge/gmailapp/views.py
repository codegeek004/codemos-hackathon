...
def delete_emails_view(request):
    try:
        ...
        if check_token_validity(creds.token) == False:
            ...
            logout(request)
            messages.warning(
                request, 'Your session was expired. login again to continue')
            return redirect('index')
        ...
        if request.method == "POST":
            ...
            try:
                task = delete_emails_task.delay(
                    request.user.id, request.user.email, category)
            ...
            messages.success(
                request, "Your email deletion request has been started.It might take several hours")
            messages.success(
                request, " You will get an email  notification upon completion.")
        ...
        print(e)
        messages.error(
            request, f"An error occurred while processing your request. {e}")
        return render(request, 'email_delete_form.html')
...
def recover_emails_from_trash_view(request):
    ...
    try:
        ...
        messages.success(
            request, 'Your email recovery is in progress. You will be notified when all emails are recovered successfully.')
        # The message will be displayed on this html page
        return render(request, 'email_delete_form.html')
