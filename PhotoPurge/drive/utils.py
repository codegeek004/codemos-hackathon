...
EXPORT_MAP = {
    ...
    # No export available — skipped downstream
...
def get_drive_files(credentials_dict: dict, page_token: str = None, page_size: int = 100):
...
def download_all_formats(src_service, file_id: str, file_name: str, mime_type: str):
    # binary file — direct download, no size limit
    ...
    # Workspace types with no export (Forms, Sites)
    if EXPORT_MAP[mime_type] is None:
        return []

    # Workspace file — export in every supported format
    results = []
    for export_mime, extension in EXPORT_MAP[mime_type]:
        buffer = io.BytesIO()
        try:
            request = src_service.files().export_media(fileId=file_id, mimeType=export_mime)
            downloader = MediaIoBaseDownload(buffer, request, chunksize=50 * 1024 * 1024)
            done = False
            while not done:
                _, done = downloader.next_chunk()
            ...
            if file_bytes:
                results.append((file_bytes, f"{file_name}{extension}", export_mime))

        except Exception:
            # exportSizeLimitExceeded or fileNotDownloadable — skip this format
            continue

    return results


def upload_drive_file(service, file_bytes: bytes, file_name: str, mime_type: str, parent_folder_id: str = None):
